...
async function createFileFromBase64(content, remotePath) {
  const base64Content = Buffer.from(content).toString('base64');
  const decodedCommand = `echo '${base64Content}' | base64 -d > ${remotePath}`;
  await runSSHCommand(decodedCommand);
}

async function simpleAutomatedDeploy() {
  console.log('🚀 Simple Automated Deployment via SSH\n');
  console.log('=======================================\n');
  ...
  try {
    ...
    await createFileFromBase64(packageJson, '~/product-card-app/package.json');
...
// Simple homepage
app.get('/', (req, res) => {
  res.send('<h1>🚀 Product Card Generator</h1><p>Server is running!</p><p><a href="/api/trpc/auth.me">Auth Endpoint</a> | <a href="/api/trpc/system.health">Health Endpoint</a></p>');
});
...
app.listen(PORT, () => {
  ...
  console.log(\`   http://localhost:\${PORT}/api/trpc/system.health\`);
    ...
    await createFileFromBase64(serverJs, '~/product-card-app/server.js');
...
~/bin/node server.js`;
    ...
    await createFileFromBase64(startupScript, '~/start-product-card.sh');
...
case "$1" in
  ...
  *)
    ...
    await createFileFromBase64(pmScript, '~/app-manager.sh');
    ...
    console.log('\n=======================================');
    console.log('🎉 SIMPLE AUTOMATED DEPLOYMENT COMPLETE!');
    console.log('=======================================\n');
    ...
    console.log('🚀 Your Product Card Generator is deployed!');
    console.log('💡 Start it with: ~/app-manager.sh start');
    
  } catch (error) {
    console.error('❌ Simple automated deployment failed:', error.message);
    throw error;
  }
}

// Run the deployment
simpleAutomatedDeploy().catch(error => {
